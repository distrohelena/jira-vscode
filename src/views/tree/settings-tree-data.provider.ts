...
export class JiraSettingsTreeDataProvider extends JiraTreeDataProvider {
	...
	protected async getSectionChildren(authInfo: JiraAuthInfo): Promise<JiraTreeItem[]> {
		this.updateBadge();
		this.updateDescription(UrlHelper.extractHost(authInfo.baseUrl));
		const nodes = JiraSettingsTreeDataProvider.buildAccountNodes(authInfo);
		return nodes;
	}

	private static buildAccountNodes(authInfo: JiraAuthInfo): JiraTreeItem[] {

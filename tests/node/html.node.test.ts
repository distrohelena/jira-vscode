...
test('sanitizeRenderedHtml strips scripts, styles and links', () => {
	...
	const sanitized = HtmlHelper.sanitizeRenderedHtml(html);
	assert.equal(sanitized, '<p>Keep me</p>');
});

test('sanitizeRenderedHtml returns undefined for empty content', () => {
	assert.equal(HtmlHelper.sanitizeRenderedHtml(''), undefined);
	assert.equal(HtmlHelper.sanitizeRenderedHtml('   '), undefined);
	assert.equal(HtmlHelper.sanitizeRenderedHtml(undefined), undefined);
});
